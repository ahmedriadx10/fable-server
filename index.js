...
async function run() {
  try {
    ...
    app.get(
      "/writer/books/:writerId",
      authorizationMiddleware,
      verifyWriterRoleMiddleware,
      async (req, res) => {
        const { writerId } = req.params;

        //prevent other writer to access another writers book
    ...
    app.get("/bookmarks/:id", authorizationMiddleware, async (req, res) => {
      const { id } = req.params;
      ...
      //prevent other user or writer to get specific user or writer bookmarks
      if (req?.user?.id !== id) {
        return res.status(403).json({ message: "Forbidden access" });
      }
    ...
    app.post("/book/purchases", authorizationMiddleware, async (req, res) => {
    ...
    app.get("/purchase/:userId", authorizationMiddleware, async (req, res) => {
      const { userId } = req.params;

      //prevent other user to access specific user purchase data
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        //prevent other user access dashboard stats data
        if (req?.user?.id !== userId) {
          return res.status(403).json({ message: "Forbidden access" });
        }
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        //prevent other writer to access writer dashboard stats
        if (req?.user?.id !== writerId) {
          return res.status(403).json({ message: "Forbidden access" });
        }

        const [totalPublished, totalBookmarked, totalSaleAmmout] =
          await Promise.all([
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        //prevent other writer to access sales history
          ...
          for (let i = 5; i >= 0; i--) {
            ...
            const foundMonth = salesAggregation.find(
              (item) =>
                ...
                item._id.month === currentMonthNum,
            );

            monthlySales.push({
              month: currentMonthName,
              sales: foundMonth
                ? parseFloat(foundMonth.totalSales.toFixed())
                : 0, // ডেটা না থাকলে ০ সেট হবে
            });
          }
    ...
    app.patch(
      ...
      async (req, res) => {
        try {
          ...
          //prevent other user to update user role
